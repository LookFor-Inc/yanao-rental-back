...
public class OAuth2AuthorizationRequestRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest> {
    ...
    public OAuth2AuthorizationRequest loadAuthorizationRequest(HttpServletRequest request) {
        return CookieUtils.getCookie(request, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME)
                .map(cookie -> Base64Utils.deserialize(cookie, OAuth2AuthorizationRequest.class))
                .orElse(null);
    }
    ...
    public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest, HttpServletRequest request, HttpServletResponse response) {
        ...
        cookieHelper.addCookie(
                ...
                Base64Utils.serialize(authorizationRequest),
                cookieExpireSeconds
        );
    ...
    public void removeAuthorizationRequestCookies(HttpServletResponse response) {
        cookieHelper.deleteCookie(response, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME);
        cookieHelper.deleteCookie(response, REDIRECT_URI_COOKIE_NAME);
    }
}
